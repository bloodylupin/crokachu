...
import { userNameInterpolation } from "./MemoryGame";
...
export default function HighScores({ placement }: HighScoresProps) {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                const res = await fetch("https://jackyolo.xyz/api/scores?game_id=test", {
                    method: "GET",
                    headers: {
                            ...
                            <ul className="p-4 bg-fuchsia-900 rounded">
                                {scores.map(({ name, score }, index) => {
                                    const match =
                                        ...
                                        index === placement ||
                                        index === placement + 1;
                                    return match ? <li key={index} className={classNames(index === placement - 1 ? "text-slate-400" : "", "grid grid-cols-7 gap-4")}>
                                        <span>
                                            {index + 1}
                                        </span>
                                        <span className="col-span-4">
                            ...
                            <ul className="p-4 bg-fuchsia-900 rounded">
                                ...
                                {account ?
                                    <li className="grid grid-cols-6 gap-4">
                                        ...
                                        <span>
                                            {scores.filter(s => s.name === userNameInterpolation(account)).length}
                                        </span>
                                    </li> : null}
                            </ul>
                        </>
