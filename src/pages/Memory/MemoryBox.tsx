...
export default function MemoryBox() {
    ...
    return (
        <>
            <div className="relative grid grid-cols-3 gap-8 rounded-md p-8 bg-gradient-to-b from-fuchsia-900 to-slate-900 shadow-lg shadow-slate-900">
                {tiles.map((tile, i) => (
                    tile !== "" ?
            ...
            <div className="flex justify-center gap-4">
                {account && favorites.length < MIN_TO_PLAY ? <><RandomSelectorButton /></> : null}
                {account && favorites.length > 0 && favorites.length < MIN_TO_PLAY ? <><ResetSelectionButton /></> : null}
            </div>
