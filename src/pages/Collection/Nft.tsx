...
export default function Nft({ uri, inputValue, setInputValue, setProperties, setMaxShow }: NftProps) {
        ...
        Object.entries(metadata).forEach(([key, value]) => {
            if (typeof value === "string") {
                ...
                value.forEach(attribute => {
                    Object.entries(attribute).forEach(([key, value]) => (key === "value") ? properties.push(value) : null);
                });
            }
        })
    ...
    const handleFavoriteSelection: MouseEventHandler<HTMLButtonElement> = (e) => {
        ...
        favorites.length === MIN_TO_PLAY && !favorites.includes(metadata.gatewayImage) ? setMaxShow(true) : null;
    }
    return (
        isVisible ?
            <>
                <div className="relative">
                    ...
                    <span className="absolute bottom-0 right-0 rounded-tl bg-purple-900/50 text-xs">
                        ...
                        <button className="w-8 h-8 block mx-auto" onClick={handleFavoriteSelection}>
                            {isFavorite ? <HeartIconSolid className="fill-lime-400" /> : <HeartIcon className="stroke-lime-400" />}
                        </button>
                    </span>
                </div>
                <Modal show={show} setShow={setShow}>
                    <div>
                        ...
                        <div className="grid mt-2 gap-2 lg:grid-cols-2 lg:gap-4 lg:mt-4">
                            {metadata.attributes.map(({ trait_type, value }, i) => {
                                return <Button key={i} className="from-fuchsia-500 to-fuchsia-900 rounded p-0" onClick={
                                    () => {
                                        setInputValue(value);
                                        setShow(false);
                                    }
